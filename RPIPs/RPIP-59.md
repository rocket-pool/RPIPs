...
- Disabling social assignments increases the likelihood of ETH accumulating in the depoist pool while validators are waiting in queue. But the node operators in queue that would be assigned ETH have a direct incentive to execute the assignments. In addition, we recommend that the pDAO funds (for example through the GMC) development and running of assignment bots that assign ETH in the deposit pool to the queue at a reasonable gas price.
